...
import com.lypaka.areamanager.API.AreaHeadbuttSpawnEvent;
            ...
            if (event.getPokeBall() instanceof OccupiedPokeBallEntity) {
                ...
                if (blockMaterial == Material.WOOD) {
                    ...
                    for (int i = 0; i < areas.size(); i++) {
                        ...
                        if (currentArea.getHeadbuttSpawnerSettings().doesUsePixelmonsSystem()) continue;
                        if (currentArea.getHeadbuttSpawnerSettings().getCooldown() > 0) {
                            ...
                            String timeExpires = now.plusSeconds(currentArea.getHeadbuttSpawnerSettings().getCooldown()).toString();
                            toSendOut.getPersistentData().putString("HeadbuttCooldown", timeExpires);

                        }
                        if (currentArea.getHeadbuttSpawnerSettings().doesRequireMove()) {
                                    ...
                                    if (!spawnEvent.isCanceled()) {
                                        ...
                                        if (spawnEvent.getToSpawn() != null) {
                                            ...
                                            if (currentArea.getHeadbuttSpawnerSettings().doesRequireMove()) {
                                                ...
                                                if (currentArea.getHeadbuttSpawnerSettings().doesReducePP()) {
                                            ...
                                            if (currentArea.getHeadbuttSpawnerSettings().getAutoBattleChance() > 0) {

                                                if (RandomHelper.getRandomChance(currentArea.getHeadbuttSpawnerSettings().getAutoBattleChance())) {
                                            ...
                                            if (currentArea.getHeadbuttSpawnerSettings().doesDespawnAfterBattle()) {
                                                ...
                                                if (currentArea.getHeadbuttSpawnerSettings().getDespawnTimer() > 0) {

                                                    pixelmon.despawnCounter = currentArea.getHeadbuttSpawnerSettings().getDespawnTimer();

                                                }

                                            }
                                            if (currentArea.getHeadbuttSpawnerSettings().doesClearSpawns()) {
                ...
                if (event.getPokeBall() instanceof OccupiedPokeBallEntity) {
                    ...
                    if (hitEntity instanceof StatueEntity) {
                        ...
                        for (int i = 0; i < areas.size(); i++) {
                            ...
                            Area currentArea = areas.get(i);
                            if (currentArea.getHeadbuttSpawnerSettings().doesUsePixelmonsSystem()) continue;
                            if (!currentArea.getHeadbuttSpawnerSettings().getCustomHeadbuttTreeIDs().contains(entityID)) continue;
                            if (currentArea.getHeadbuttSpawnerSettings().getCooldown() > 0) {

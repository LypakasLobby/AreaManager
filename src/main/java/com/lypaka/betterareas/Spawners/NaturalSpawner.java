...
import com.lypaka.betterareas.API.AreaNaturalSpawnEvent;
...
import com.lypaka.betterareas.Areas.Spawns.NaturalSpawn;
...
public class NaturalSpawner {
    ...
    private static Map<Area, Integer> spawnAttemptMap = new HashMap<>();
            ...
            public void run() {
                ...
                for (Map.Entry<Area, List<UUID>> entry : AreaHandler.playersInArea.entrySet()) {
                    ...
                    if (entry.getValue().size() > 0) {
                        ...
                        if (spawnIntervalCount < area.getNaturalSpawnerSettings().getSpawnInterval()) {
                        ...
                        for (UUID uuid : entry.getValue()) {
                            ...
                            for (int i = 0; i < areas.size(); i++) {
                                ...
                                if (currentArea.getNaturalSpawnerSettings().doesLimitSpawns()) {
                                ...
                                if (spawns.getNaturalSpawns().size() > 0) {
                                    ...
                                    if (!event.isCanceled()) {
                                        ...
                                        for (Pokemon poke : toSpawn) {
                                            ...
                                            if (currentArea.getNaturalSpawnerSettings().doesClearSpawns()) {
                                            ...
                                            player.world.getServer().deferTask(() -> {
                                                ...
                                                if (currentArea.getNaturalSpawnerSettings().getDespawnTimer() > 0) {

                                                    pixelmon.despawnCounter = currentArea.getNaturalSpawnerSettings().getDespawnTimer();

                                                }
                                                if (toSpawn.size() == 1) {

                                                    // only one Pokemon spawned, so we check for the auto battle stuff
                                                    if (RandomHelper.getRandomChance(currentArea.getNaturalSpawnerSettings().getAutoBattleChance())) {
                                                        ...
                                                        if (!player.isCreative() && !player.isSpectator()) {
                                                            ...
                                                            if (BattleRegistry.getBattle(player) == null) {
                                                                ...
                                                                String message = currentArea.getNaturalSpawnerSettings().getMessagesMap().get(messageType);
